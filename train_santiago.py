import os
os.environ['WANDB_CACHE_DIR']='/gpfsstore/rech/qpj/uyn98cq/WANDB_CACHE/'
import argparse
...
from hpcs.hyp_hc_santiago import SimilarityHypHC
...
from hpcs.nn.dgcnn import VN_DGCNN_santiago
...
def read_configutation():
    ...
    parser.add_argument('--hierarchical', '-hierarchical', default=False, type=bool, help='')
    parser.add_argument('--wandb','-wandb',default='online',type=str, help='Online/Offline WandB mode (Useful in JeanZay)')
    parser.add_argument('--normalize_classification','-normalize_classification',default=True,type=bool,help='Norm to one in W for classification')
    args = parser.parse_args()
    return args

def configure(args):
    #Add to wandb parameters
    ...
    normalize_classification=args.normalize_classification
    ...
    if dataset == 'shapenet':
        ...
        test_loader2 = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=num_workers, drop_last=True)
    ...
    elif model_name == 'vn_dgcnn_santiago':
        nn = VN_DGCNN_santiago(in_channels=3, out_features=out_features, k=k, dropout=dropout, num_class=num_class,normalize_classification=normalize_classification)
    ...
    logger = WandbLogger(name=dataset, save_dir=os.path.join(log), project='HPCS', log_model=True,allow_val_change=True)
    ...
    #logger.experiment.config.update(model_params)
...
def train(model, trainer, train_loader, valid_loader, test_loader, resume):
    ...
    print('Using test_loader')
    for i in range(2):
        trainer.test(model, test_loader)
    print('Just to be sure that the results are equal')
